...
getRequest(`${SW_API}/people/2`).pipe(
  ...
  map<Person, string>(p => p.species ? p.species[0] : ''),
  mergeMap<string, Observable<Specie>>(url => getRequest(url))
...
// Solution 2
getRequest(`${SW_API}/people/2`).pipe(
  mergeMap<Person, Observable<any>>(person =>
    zip(of(person), getRequest(person.species[0])).pipe(
      map(([specie, person]) => ({ specie, person }))
    ))
).subscribe(console.log);         